...
class QuircClientThread(QThread):
	...
	def pubchat(self,sender,**kw):
		...
		#self.chatlog[target].addChat(f"{nick}: {msg}")
	...
	def join_channel(self,sender,**kw):
		...
		if nick == self.nickname:
			self.channels[channel] = QChannel()
	...
	def welcome(self,sender,**kw):
		# Store the IRC connection object for later
	...
	def run(self):
		...
		tirc = QuircClient()
		#tirc.connect(tirc.pubmsg,self.gotti )
		try:
...
class QuircClient(irc.client.SimpleIRCClient):

	#pubmsg = pyqtSignal(str)
	...
	def on_notopic(self,connection,event):
		#print(f"NO TOPIC {event.source}")
		pass

	def on_currenttopic(self,connection,event):
		#print(f"CURRENT TOPIC {event.source} {event.target} {event.arguments}")
	...
	def on_topicinfo(self,connection,event):
		#print(f"TOPIC INFO {event.source} {event.target} {event.arguments}")

		topic_event = signal("topicsetter")
		topic_event.send(connection,channel=event.arguments[0],nick=event.arguments[1])

	def on_topic(self,connection,event):
		#print(f"FLARRRRP TOPIC INFO {event.source} {event.target} {event.arguments}")
		user = NickMask(event.source)
